...
const isValidFormInput = (postUrl: string) => {
  if (!postUrl) {
    return "Facebook URL was not provided";
  }

  if (!postUrl.includes("facebook.com")) {
    return "Invalid URL does not contain Facebook domain";
  }
...
export default function FacebookForm({ onValueClear, onValueChange }: any) {
  ...
  async function onSubmit(setUrl?: string) {
    ...
    try {
      ...
      const fbPath = getPathApiFromPlatform("facebook");

      const response: any = await fetchVideoInfo({
        apiUrl: `${process.env.NEXT_PUBLIC_API_URL}/${fbPath}`,
    ...
    >
      ...
      <div className="flex w-full flex-col items-center gap-2 md:flex-row">
        <InputField
          ...
          aria-label="Facebook video download URL input"
          title="Facebook video download URL input"
          value={postUrl}
          onPaste={onPaste}
