...
export const CardVideoShortInfo = ({ videoInfo }: any) => {
  const clearResult = () => {
    ...
    <>
      {videoInfo.thumb && (
        <section className="container mx-auto flex flex-wrap justify-center gap-5 px-5 py-20 md:col-span-2 xl:col-span-1">
          <div className=" video-info z-10 flex rounded-md bg-gray-200">
            <div className="flex-none rounded-md p-2">
              <div className="h-[184px] md:w-full">
                <Image
                  unoptimized={true}
                  width={103}
                  height={184}
                  ...
                  className="card-img-top img-fluid h-full w-auto rounded-md"
                />
              </div>
            </div>
            <div className="w-[300px] flex-auto p-2">
          ...
          <div className="block">
            {videoInfo.formats.map((item: Format, index: number) => (
              <div key={index}>
                <Button
                  color="success"
                  onClick={() => downloadFile(item.name, item.url)}
                  startContent={
                    <svg
                      stroke="currentColor"
                      fill="currentColor"
