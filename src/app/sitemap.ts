...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  await Promise.all(languages.map(async (locale) => {
    const jsonData = fs.readFileSync(`${rootPath}/messages/${locale}.json`, "utf-8");
    const parsedData: any = JSON.parse(jsonData);

    const keys = Object.keys(parsedData.Page);

    const sitemapItem = keys.map(key => {
      const slug = !parsedData.Page[key]["slug-high-level"] && parsedData.Page[key].title ? `/${slugify(parsedData.Page[key].title)}` : "";
      return {
