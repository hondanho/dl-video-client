...
export default function InstagramReelsPage() {
  const path = "download-instagram-reels";
  const t = useTranslations(`Page.${path}`);
  const base = useTranslations("Page.base");

  const structuredHowToJson = structuredHowTo(t, base, path);
  const structuredSoftwareApplicationJson = structuredSoftwareApplication(t, path, 58008);
