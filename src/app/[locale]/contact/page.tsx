  ...
  return (
    <div className="mx-auto my-8 h-fit flex-1 bg-white p-4 shadow-sm md:rounded mb-20 container">
      <div className="flex flex-col text-center w-full mb-12">
        <h1 className="text-2xl mb-2 font-bold text-black">{t("title")}</h1>
        <p className="lg:w-2/3 mx-auto leading-relaxed text-base">{t("#1")} <a className="hover:text-decoration-underline" href={`mailto:${process.env.NEXT_PUBLIC_EMAIL}`}>{process.env.NEXT_PUBLIC_EMAIL}</a></p>
      </div>
      <section className="text-gray-600 body-font relative">
        ...
        <div className="container px-5 py-24 mx-auto flex">
          {!suscess && <form className="lg:w-1/2 md:w-1/2 bg-white rounded p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0 relative z-10 shadow-md" onSubmit={handleSubmit}>
            <h2 className="text-gray-900 text-lg mb-1 font-medium title-font">{t("#2")}</h2>
            {/* <p className="leading-relaxed mb-5 text-gray-600">Post-ironic portland shabby chic echo park, banjo fashion axe</p> */}
            <div className="relative">
              <label className="leading-7 text-sm text-gray-600">{t("#3")}</label>
            ...
            <div className="relative mb-4">
              <label className="leading-7 text-sm text-gray-600">{t("#4")}</label>
            ...
            <button className="text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg" type="submit">{t("#5")}</button>
            {/* <p className="text-xs text-gray-500 mt-3">Chicharrones blog helvetica normcore iceland tousled brook viral artisan.</p> */}
          </form>}
          {suscess &&
            <div className="lg:w-1/2 md:w-1/2 bg-white rounded p-8 min-h-[300px] flex flex-col md:ml-auto w-full mt-10 md:mt-0 relative z-10 shadow-md text-center text-green-500">{t("#6")}</div>
          }
        </div>
      </section>
    </div>
